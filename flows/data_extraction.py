...
def get_pdf_urls(url):
    ...
    try:
        ...
        logger.info("Error fetching PDF URLs... " + url)
        return set()
...
def get_urls(url):
    ...
    try:
        ...
        logger = get_run_logger()
        logger.info(f"Error requesting URL... {url} - {e}")
        return
    if r.status_code != 200:
        return set()
...
def download_pdfs_from_url_recursive(
    ...
    if remaining_levels == original_levels and print_details:
        logger.info(
            f"Depth Level 0 (Main Source) -> {len(pdf_urls)} PDFs found -> Source: {url}"
        )
    ...
    for i, url_inside in enumerate(other_urls):
        ...
        if print_details:
            logger.info(
                "..." * (depth_level - 1)
                + f"Depth Level {depth_level} -> {i + 1}/{len(other_urls)} URLs -> Source: {url_inside}"
            )
